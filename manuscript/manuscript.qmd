...
tbl_data |>
  select(translation, accuracy, accuracy_se) |> 
  mutate(accuracy = round(accuracy * 100, 2),
         accuracy_se = round(accuracy_se * 100, 2)) |> 
  set_names(c("Translation", "Accuracy (%)", "SE")) |> 
  tt() |> 
  group_tt(i = list(
    "Experiment 1 (cat-ENG)" = 5,
    "Experiment 2 (spa-ENG)" = 21,
    "Experiment 3 (cat-SPA)" = 26))
```
