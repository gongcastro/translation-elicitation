...
tbl_data <- list(
  ...
  mutate(
    ...
    xsampa = paste0("/", sampa_1, "/ - /", sampa_2, "/")
  ) |> 
  summarise(
    n = n(),
    correct = sum(correct),
    .by = c(exp, group, lv, translation, xsampa)
  ) |>
  mutate(
  ...
  select(exp, translation, xsampa, group, accuracy, accuracy_se) |>
  arrange(exp, group, desc(accuracy)) |> 
  select(translation, xsampa, accuracy, accuracy_se) |> 
  mutate(accuracy = round(accuracy * 100, 2),
         accuracy_se = round(accuracy_se * 100, 2))

names(tbl_data) <- c("Translation", "X-SAMPA", "Accuracy (\\%)", "SE")
