...
\renewcommand{\runninghead}{Cognateness and native word recognition }
...
\citeproc{ref-takata1990english}{Takata \& Nábělek, 1990}). One of the
sources of this increased difficulty is the partial mismatch between the
phonology of the native and the non-native languages: some acoustic
features embedded in the non-native speech signal do not overlap
perfectly with any phonemic category in the listener's native language.
For example, imagine a Spanish native with no previous familiarity with
any other language listening to French for the first time. This listener
may encounter the word /pɔʁt/ (\emph{porte}), translation of door in
French. The voiced uvular fricative consonant /ʁ/ and the open-mid back
rounded vowel /ɔ/ do not exist in Spanish as phonemic categories. These
discrepancies can nonetheless be perceived as allophonic variations the
native (Spanish) phonemes /ɾ/ and /o/
(\citeproc{ref-best2001discrimination}{Best et al., 2001}), leading to
the non-native speech signal engaging lexical processing mechanisms
(\citeproc{ref-weber2004lexical}{Weber \& Cutler, 2004}). Even if
unfamiliar with the word-form /pɔʁt/, the Spanish monolingual is likely
to activate the phonological representation of its Spanish translation
/pweɾta/ (\emph{puerta}), ultimately accessing the right semantic
representation. Although the phonetic mismatch between the non-native
word and its translation can have a noticeable toll on comprehension
...
completely naïve to the language they are listening to.

Imagine now that the same Spanish listener hears English for the first
time and encounters the word /dɔː/ (\emph{door}). This time,
phonological similarity is of little help: /pweɾta/ and /dɔː/ do not
share any phonemes. It would come as a surprise if the listener was able
to translate the English word successfully. Non-native listeners rely on
the phonological similarity between the word-forms presented in the
input and their translations. Luckily for them, most languages share
such kind of similarity. This is frequently due to their typological
...
translation equivalents (also known as \emph{cognates})
...
respectively). This type of cross-linguistic similarity has been
extremely informative for the study of bilingual lexical processing:
words that share high form-similarity with their translations are
processed faster and more accurately than words sharing lower
phonological similarity. This phenomenon has provided strong evidence in
favour of the language-non selective hypothesis of bilingual lexical
access, which states that bilinguals activate lexical representations in
both languages, even during monolingual situations. This facilitation
effect has been reported for comprehension
...
(\citeproc{ref-christoffels2006memory}{Christoffels et al., 2006}). The
role that cross-linguistic phonological similarity might play in
monolingual lexical access during non-native speech comprehension is
unclear. How much phonological similarity is enough for a monolingual to
understand a word from a non-native language? How does phonological
similarity interact with other determinants of word recognition?

In the present study, we investigate the extent to which monolinguals
can exploit phonological similarity when listening to words in an
unfamiliar languages, in order to achieve comprehension, while taking
into account other lexically-relevant variables interact with the
lexical processing mechanisms involved. We used a translation
elicitation task in which participants listened to words from an
unfamiliar language, and then tried to guess their translation in their
native language. We will henceforth refer to the auditory-presented
words heard by participants on each trial as \emph{presented words}, and
the correct translation for the presented words as \emph{target
translations}. We explored listeners' reliance on phonological
...
language, they could only be able to use word-to-word connections to
succeed in the translation of words in an unfamiliar language. We
therefore predicted that, if the information provided by the
...
The extent to which participants can rely on phonological similarity is
limited by other factors. In particular, the presence of phonological
neighbours (word-forms than differ in one phoneme) may interfere with
the activation and selection of the appropriate lexical representation
...
specially true if such neighbours share higher phonological similarity
...
(\citeproc{ref-weber2004lexical}{2004}) observed that bilinguals
displayed lexical competition effects from words in their native
language when performing a word-referent matching task in their L2.

In the case of the present task, the potential effect of cross-language
phonological neighbourhood density is critical. The presence of
phonologically similar non-translation equivalents (e.g., false friends)
may act as distractor during lexical access, obstructing the selection
of the appropriate target translation in the listener's lexicon. Even if
...
presented word also shares high phonological similarity with words on
the target language other than its translation, specially if any of them
is more lexically frequent than the target translation. We hypothesised
that the size of the facilitatory role of phonological similarity would
be inversely proportional to the amount of higher-frequency phonological
neighbours of the presented word in the target language. For instance,
...
closer). To test this prediction, we developed a lexical
frequency-dependent measure of cross-language phonological neighbourhood
density, in which a neighbour is counted only if it is higher frequency
and is one phoneme appart from the presented word. If the phonological
neighbourhood density of the target translation affects participants'
performance negatively, this would suggest that non-native words trigger
competition between words in the native language during translation.
...
translating words from typologically close and distant languages, we
...
performance. Participants in Experiments 1 and 2 were surprisingly good
at translation some words, which had low phonological similarity with
their correct translation. In Experiment 3, we collected additional data
from a new group of British English natives, who were presented with
Catalan and Spanish words. In contrast with the procedure in Experiment
1, participants in Experiment 3 where asked, after providing a response
in each trial, to report whether they had previous knowledge of the
meaning of the presented word (e.g., some Spanish words are common
knowledge, even for monolingual speakers of English). Finally, we
present analyses on the joint data sets of Experiments 1 to 3.
