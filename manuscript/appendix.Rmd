...
posterior_cp <- as.array(model_fits$fit_4)
np_cp <- nuts_params(model_fits$fit_4)

plot_pairs <- mcmc_pairs(
  posterior_cp, pars = pars,
  off_diag_args = list(size = 0.75),
  diag_args = list(facet_args = list(labeller = label_value(labels = pars_labels)))
)
...
(mcmc_trace(model_fits$fit_4, pars) + legend_none()) /
  mcmc_rank_overlay(model_fits$fit_4, pars = pars) +
...
rhats <- rhat(model_fits$fit_4)
ratios <- neff_ratio(model_fits$fit_4)
...
include_graphics(here("Figures", "accuracy_spacat.png"))
