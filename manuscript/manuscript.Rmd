...
pthn_example <- clearpond_path %>% 
  ...
  bind_rows(.id = "group") %>% 
  clean_names() %>% 
  filter(word %in% c("book", "sock"), group=="ENG-CAT") %>% 
  select(word, pthn) 

```

#### Target translatiuon phonological neighbourhood density (PTHN)
...
d <- stimuli %>% 
  mutate(
    ...
    group = factor(
      group,
      levels = c("cat-ENG", "spa-ENG", "cat-SPA")
    )
  ) 
...
sds <- gather_draws(fit_6, `sd_.*`, regex = TRUE) %>% 
  mean_qi() %>% 
  mutate(
    .value = ifelse(
      grepl("Intercept", .variable), 
...
h_cor_intercept_frequency <- get_bf("participant__Intercept__frequency_zipf_std = 0", class = "cor")
h_cor_frequency_zipf_lv_group1 <- get_bf("participant__frequency_zipf_std__lv_std:groupENGvs.SPA = 0", class = "cor")
h_cor_intercept_pthn <- get_bf("participant__Intercept__pthn_std = 0", class = "cor")
```
