...
get_responses_processed <- function(
    ...
    processed <- responses_path %>% 
        ...
        mutate( 
            ...
            city = ifelse(participant == "55lvve0vwnrihv0rihqipejv1nfnr", "Málaga", city),
            ...
            demo_impairment = !(demo_impairment %in% "N"),
            # group participants by native language and testing language
            group = case_when(
                country %in% "UK" & test_language %in% "Catalan" ~ "ENG-CAT",
                country %in% "UK" & test_language %in% "Spanish" ~ "ENG-SPA",
                TRUE ~ "SPA-CAT"
...
#### participant data ----------------------------------------------------------
get_participants <- function(responses_processed, responses_coded, save = TRUE){
    participants <- responses_coded %>% 
        rowwise() %>% 
    ...
    if (save) saveRDS(participants, here("results", "participants.rds"))
...
# processed responses
get_responses <- function(responses_coded, practice_trials, participants, stimuli, save = TRUE){
    ...
    if (save) saveRDS(responses, here("results", "responses.rds"))
    
    return(responses)
}

>>>>>>> c92465ffee5b01a15c56bc645bd58fd50fe89122:R/01_responses.R
