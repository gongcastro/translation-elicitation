...
#'
get_que_raw <- function(que_raw_files) {
  # process raw data
  que_raw <- file.path(que_raw_files) |>
    ...
    rename_with(\(x) str_remove(x, "_confidence"), starts_with("confidence")) |>
    rename(
...
#'
get_que_processed <- function(que_raw) {
  ...
  que_processed <- que_raw |>
    mutate(
      group = case_when(
        l1 == "English" & language == "Catalan" ~ "cat-ENG",
        l1 == "English" & language == "Spanish" ~ "spa-ENG",
        l1 == "Spanish" & language == "Catalan" ~ "cat-SPA"
      ),
      language = str_remove(language, "questionnaire_"),
      consent = consent ==
        "I acknowledge that this box indicates my digital signature.",
      ...
      across(c(starts_with("knowledge_"), ends_with("_problems")), \(x) {
        x == "Yes"
      }),
      across(c(l1, l2, l3), split_languages),
      age = as.integer(age),
      across(c(ends_with("_prod"), ends_with("_comp")), \(x) {
        as.integer(unlist(str_extract_all(x, "(\\d)+")))
      })
    ) |>
    relocate(
      matches("knowlegde_|translation_|confidence_"),
      .after = last_col()
    ) |>
    dplyr::filter(finished, consent, captcha) |>
    pivot_longer(
      -c(participant_id:has_vision_problems),
    ...
    mutate(
      ...
      group = case_when(
        language == "Catalan" & l1 == "English" ~ "cat-ENG",
    ...
    relocate(
      all_of(c("knowledge", "translation", "confidence")),
      .after = last_col()
    ) |>
...
get_que_participants <- function(
  ...
  que_participants <- read_csv(
    ...
    mutate(
      valid_response = response_type %in% valid_codes,
  ...
  return(que_participants)
}


#' Make dataset with responses from Experiment 2
#'
get_que_responses <- function(que_processed, que_participants, stimuli) {
  ...
  que_responses <- read_csv(responses_coded_path, col_types = "cccclicc") |>
    rename(participant_id = participant) |>
    left_join(
      select(que_participants, participant_id, group, valid_status),
  ...
  arrow::write_csv_arrow(que_responses, out_path)
  cli_alert_success("Saved {.emph que_responses} as {.file {out_path}}")

  return(que_responses)
}
