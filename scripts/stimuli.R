...
import_clearpond <- function(
  ...
  suppressWarnings({
    header <- readLines(cp_h_path_eng, warn = FALSE)
    eng <- read_tsv(
    ...
    header <- readLines(cp_h_path_spa, warn = FALSE)
    spa <- read_tsv(
    ...
    out <- bind_rows(lst("English" = eng, "Spanish" = spa), .id = "language") |>
      rename(sampa = phono, freq = frequency) |>
      mutate(
...
get_duration <- function(trial_list, audios_path) {
  trials <- trial_list |>
    ...
    mutate(file = file.path(audios_path, file))
