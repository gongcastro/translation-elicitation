...
get_stimuli <- function(trial_list, levenshtein, corpus,
    ...
    stimuli <- trial_list |> 
        ...
        select(group, target_language, translation, translation_id, word_1, word_2, 
               sampa_1, sampa_2, ipa_flat_1, ipa_flat_2,
               freq_zipf_2, lv, lv_dist,
               duration, practice_trial) |> 
        dplyr::filter(!(word_1 %in% stimuli_exclude)) |> 
        mutate(avg_sim = get_avg_sim(word_1 = sampa_1,
                                     word_2 = sampa_2,
                                     freq_2 = freq_zipf_2,
                                     language_2 = target_language,
                                     corpus = corpus_tmp),
               avg_sim_h = get_avg_sim(word_1 = sampa_1,
                                       word_2 = sampa_2,
                                       freq_2 = freq_zipf_2,
                                       language_2 = target_language,
                                       corpus = corpus_tmp,
                                       higher_frequency = TRUE),
               neigh_lst = get_neighbours(word_1 = sampa_1,
                                          word_2 = sampa_2,
                                          freq_2 = freq_zipf_2,
                                          language_2 = target_language,
                                          corpus = corpus_tmp),
               neigh_lst_h = get_neighbours(word_1 = sampa_1,
                                            word_2 = sampa_2,
                                            freq_2 = freq_zipf_2,
                                            language_2 = target_language,
                                            corpus = corpus_tmp,
    ...
    out_stimuli <- stimuli |> 
        rowwise() |> 
        mutate(across(matches("neigh_lst"),
