...
```{r}
#| label: tbl-stimuli
tab_subtitle <- md("Lexical frequency, phonological neigbourhood size, and levenshtein distance of the stimuli words.")

tab_note <- md("*Note*. Median and standard deviation (*SD*) of lexical frequencies of target words (expressed in counts per million and as Zipf scores), density of higher-frequency phonological neighbourhoods of target words, and normalised Levenshtein phonological similarity between the presented and the target words.")
...
stimuli %>% 
    ...
    tab_header(title = md("Table 1"), subtitle = tab_subtitle) %>% 
    tab_source_note(tab_note) %>% 
...
stimuli %>% 
    ...
    ggplot() +
    aes(x = lv, y = reorder_within(translation, lv, group), fill = group, colour = group) +
    facet_wrap(~group, scales = "free_y") +
    geom_text(aes(label = percent(lv, accuracy = 0.1)), size = 3, hjust = 0) +
    labs(
        x = "Group", 
...
empirical_accuracy <- responses %>% 
    mutate(translation = paste0(word_1, " /", ipa_flat_1, "/ - ",  word_2, " /", ipa_flat_2, "/ - ", percent(lv, accuracy = 1))) %>% 
...
post_re <- add_epred_draws(nd_re_words, fit_4, ndraws = 50,
...
post_re %>% 
    ggplot() +
    aes(x = .mean, y = reorder_within(translation, prop, group)) +
    facet_wrap(~group, scales = "free_y") +
    geom_vline(data = empirical_means, aes(xintercept = prop), size = 0.75, colour = "grey") +
    geom_errorbar(aes(xmin = .lower, xmax = .upper), size = 0.5, colour = "grey", width = 0.5) + 
    geom_point(aes(x = .mean), size = 0.5, colour = "grey") + 
    geom_point(aes(x = prop, colour = group), size = 1, shape = 15) +
    labs(
...
nd <- expand.grid(
    ...
    freq_zipf_2_std = 0,
    nd_std = scale_values(c(0, 3, 6, 10, 15, 25), fit_4$data$nd)
)

m <- add_epred_draws(nd, fit_4, re_formula = NA, ndraws = 50) %>% 
    mutate(nd_lab = rescale_values(nd_std, fit_4$data$nd) %>% 
               paste0(" neighbours") %>% 
               factor(levels = paste0(c(0, 3, 6, 10, 15, 25), " neighbours"))
    )

ggplot(m, aes(x = lv_std, y = .epred)) +
    facet_wrap(~nd_lab, nrow = 1) +
    geom_line(aes(group = interaction(nd_std, .draw)),
              colour = "grey", size = 0.75, alpha = 0.25) +
...
post_draws <-  gather_draws(questionnaire_fit_4, `b_.*`, `sd_.*`, regex = TRUE) 
...
questionnaire_nd <- expand.grid(
    ...
    freq_zipf_2_std = 0,
    nd_std = scale_values(c(0, 3, 6, 10, 15, 25), questionnaire_fit_4$data$nd)
)

questionnaire_m <- add_epred_draws(nd, questionnaire_fit_4, re_formula = NA, ndraws = 50) %>% 
    mutate(nd_lab = rescale_values(nd_std, questionnaire_fit_4$data$nd) %>% 
               paste0(" neighbours") %>% 
               factor(levels = paste0(c(0, 3, 6, 10, 15, 25), " neighbours"))
    )
