...
fit_responses <- readRDS(here("Results", "fit_responses_4.rds"))
...
map(loo$loos, "estimates") %>% 
  ...
  mutate(formula = c(
    ...
    "~ 1 + frequency + pthn*vowel_ratio + (1 + pthn*vowel_ratio | participant)",
    "~ 1 + frequency + pthn*consonant_ratio + (1 + pthn*consonant_ratio  | participant)",
    "~ 1 + frequency + pthn*(consonant_ratio + consonant_ratio) + (1 + pthn*(consonant_ratio + consonant_ratio) | participant)"
  )) %>% 
  arrange(desc(model)) %>% 
...
ggplot(m, aes(consonant_ratio, .value, colour = interaction(vowel_ratio, pthn, sep = "/"), fill = interaction(vowel_ratio, pthn, sep = "/"))) +
  ...
  ggsave(here("Figures", "responses_consonant.png"), height = 4, width = 4.5)
...
ggplot(m, aes(vowel_ratio, .value, colour = interaction(consonant_ratio, pthn, sep = "/"), fill = interaction(consonant_ratio, pthn, sep = "/"))) +
  ...
  ggsave(here("Figures", "responses_vowel.png"), height = 4, width = 4.5)
