...
fit_responses <- readRDS(here("Results", "fit_responses.rds"))
loo <- readRDS(here("Results", "loo.rds"))
...
Participants in the ENG-SPA listened to `r nrow(trials[trials$group=="ENG-SPA",])` Spanish words, and participants in the ENG-CAT and the SPA-CAT groups listened to `r nrow(trials[trials$group=="ENG-CAT",])` trials. Five trials in each condition were practice trials and were excluded from analyses.
...
We also added participants (`participants`) and as a grouping variable (aka., random effects), therefore adding participant-level adjustments to the model. We specified random intercepts slopes for all fixed effects within the `participant` grouping variable.
...
```
~ pthn + consonant_ratio + vowel_ratio + onset + (1 + pthn + consonant_ratio + vowel_ratio + onset | participant)
```
...
* fit_2: `~ pthn + consonant_ratio + vowel_ratio + (1 + pthn + consonant_ratio + vowel_ratio | participant)`
* fit_3: `~ pthn + consonant_ratio + (1 + pthn + consonant_ratio | participant)`
* fit_4: `~ pthn + (1 + pthn | participant)`
* fit_5: `~ 1 + (1  | participant)`
...
summary(fit_responses)$fixed %>% 
  ...
  tab_footnote(
    ...
    locations = cells_body(columns = "term", rows = term %in% c("pthn", "consonant_ratio", "vowel_ratio", "onset1"))
  ) %>% 
  tab_footnote(
...
map(loo$loos, "estimates") %>% 
  ...
  bind_rows(.id = "model") %>% 
  clean_names() %>% 
  filter(ic %in% c("elpd_loo", "looic")) %>% 
  pivot_wider(names_from = ic, values_from = c("estimate", "se")) %>% 
  left_join(rownames_to_column(as.data.frame(loo$comparison, "model"))) %>% 
  mutate(formula = c(
    "... + onset + (0 + onset | participant)",
    "... + vowel_ratio + (0 + vowel_ratio | participant)",
    "... + consonant_ratio + (0 + consonant_ratio | participant)",
    "... + pthn + (0 + pthn | participant)",
    "~ 1 + (1  | participant)"
  )) %>% 
  arrange(desc(model)) %>% 
  select(formula, estimate_elpd_loo, se_elpd_loo, estimate_looic, se_looic, elpd_diff, se_diff) %>%
  rename(model = formula) %>% 
  gt() %>% 
  fmt_number(2:7) %>% 
  cols_merge(c("estimate_elpd_loo", "se_elpd_loo"), pattern = "{1} ({2})") %>% 
  cols_merge(c("estimate_looic", "se_looic"), pattern = "{1} ({2})") %>% 
  cols_merge(c("elpd_diff", "se_diff"), pattern = "{1} ({2})") %>% 
  cols_label(
    model = md("**Model**"),
    estimate_elpd_loo = md("**ELPD<sub>LOO</sub> (SE)**"),
    estimate_looic = md("**LOO-IC (SE)**"),
    elpd_diff = md("**ELPD<sub>diff</sub> (SE)**")
...
#### Marginal effects {.tabset .tab-pills}

##### By PTHN

```{r emmeans_pthn, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
nd <- expand_grid(
  pthn = seq(
...
m <- add_fitted_draws(nd, fit_responses, n = 20, re_formula = NA) %>% 
  mutate_at(vars(ends_with("_ratio")), function(x) paste0(x, " SD")) %>% 
  mutate(
    overlap_stress = paste0("Stress: ", overlap_stress),
    onset = paste0("Onset: ", onset)
  )
...
ggplot(m, aes(pthn, .value, 
  ...
  ggsave(here("Figures", "responses_pthn.png"))

```


##### By consonant similarity

```{r emmeans_consonant, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
nd <- expand_grid(
  pthn = c(-1, 1),
  onset = unique(responses_coded$onset),
  overlap_stress = unique(responses_coded$overlap_stress),
...
m <- add_fitted_draws(nd, fit_responses, n = 20, re_formula = NA) %>% 
  mutate_at(vars(vowel_ratio, pthn), function(x) paste0(x, " SD")) %>% 
  mutate(
    overlap_stress = paste0("Stress: ", overlap_stress),
    onset = paste0("Onset: ", onset)
  )

ggplot(m, aes(consonant_ratio, .value, 
              colour = interaction(pthn, vowel_ratio, sep = "/"),
              fill = interaction(pthn, vowel_ratio, sep = "/"))) +
  facet_wrap(~onset) +
  # stat_lineribbon(.width = 0.95, alpha = 0.25, size = 0) +
  stat_summary(fun = mean, geom = "line", size = 1) +
  labs(x = "Consonant similarity (SD)",
       y = "P(Correct)", colour = "PTHN/Vowel similarity",
  ...
  ggsave(here("Figures", "responses_consonant.png"))
...
```{r emmeans_vowel, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
nd <- expand_grid(
  pthn = c(-1, 1),
  onset = unique(responses_coded$onset),
  overlap_stress = unique(responses_coded$overlap_stress),
...
m <- add_fitted_draws(nd, fit_responses, n = 20, re_formula = NA) %>% 
  mutate_at(vars(consonant_ratio, pthn), function(x) paste0(x, " SD")) %>% 
  mutate(
    overlap_stress = paste0("Stress: ", overlap_stress),
    onset = paste0("Onset: ", onset)
  )

ggplot(m, aes(vowel_ratio, .value, 
              colour = interaction(pthn, consonant_ratio, sep = "/"),
              fill = interaction(pthn, consonant_ratio, sep = "/"))) +
  facet_wrap(~onset) +
  # stat_lineribbon(.width = 0.95, alpha = 0.25, size = 0) +
  stat_summary(fun = mean, geom = "line", size = 1) +
  labs(x = "Vowel similarity (SD)",
       y = "P(Correct)", colour = "PTHN/Consonant similarity",
  ...
  ggsave(here("Figures", "responses_vowel.png"))
```
...
ggplot(responses_item, aes(reorder(word_label, correct_prop), correct_prop, ymin = correct_prop-correct_se, ymax = correct_prop+correct_se, colour = group)) +
  ...
  ggsave(here("Figures", "accuracy_item.png"))
...
ggplot(responses_participant, aes(reorder(participant_label, correct_prop), correct_prop, ymin = correct_prop-correct_se, ymax = correct_prop+correct_se, colour = group)) +
  ...
  ggsave(here("Figures", "accuracy_participant.png"))
...
```{r jtrace_all_plot, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
ggplot(jtrace, aes(x = activation, y = pct_participant)) +
  geom_smooth(colour = wes_palettes$Darjeeling1[1],
              fill = wes_palettes$Darjeeling1[1],
              alpha = 0.5) +
  ...
  ggsave(here("Figures", "trace_average.png"))
...
ggplot(re, aes(Estimate, input, colour = input)) +
  ...
  geom_errorbarh(aes(xmin = Q2.5, xmax = Q97.5), height = 0, size = 1) +
  geom_point() +
