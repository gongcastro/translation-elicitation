...
fit_responses <- readRDS(here("Results", "fit_responses_2.rds"))
...
```{r trials_similarity_onset, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
trials_coded %>% 
...
map(loo$loos, "estimates") %>% 
  ...
  mutate(formula = c(
    "~ 1 + frequency + pthn*lv + (1 + pthn*lv | participant)",
    "~ 1 + frequency + pthn + (1 + pthn | participant)",
    "~ 1 + frequency + (1 | participant)"
  )) %>% 
  arrange(desc(model)) %>% 
...
ggplot(m, aes(pthn, .value, colour = lv, fill = lv)) +
  ...
  labs(x = "PTHN", y = "P(Correct)", colour = "Phon. similarity", fill = "Phon. similarity") +
...
```{r emmeans_lv, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.width=6, fig.height=6}
# by lv
nd <- expand_grid(
  pthn = c(-1, 1),
  frequency = 0,
...
ggplot(m, aes(lv, .value, colour = pthn, fill = pthn)) +
  stat_lineribbon(.width = 0.95, alpha = 0.25, size = 0) +
  stat_summary(fun = mean, geom = "line", size = 1) +
  labs(x = "Phon. similarity", y = "P(Correct)", colour = "PTHN", fill = "PTHN") +
  scale_y_continuous(labels = scales::percent, limits= c(0, 1)) +
  guides(colour = guide_legend(ncol = 2)) +
  theme_custom() +
  theme(
  ...
  ggsave(here("Figures", "responses_lv.png"))
