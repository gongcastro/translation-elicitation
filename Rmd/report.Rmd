...
fit_responses <- readRDS(here("Results", "fit_responses_3.rds"))
...
* fit_0: `~ 1 + (1  | participant)`
* fit_1: `~ pthn + (1 + pthn | participant)`
* fit_2: `~ pthn*lv + (1 + pthn*lv | participant)`
...
map(loo$loos, "estimates") %>% 
  ...
  mutate(formula = c(
    "...+onset+pthn:onset+lv:onset+pthn:lv:onset+(0+onset+pthn:onset+lv:onset+pthn:lv:onset|participant)",
    "...+lv+pthn:lv+(0+lv+pthn:lv|participant)",
...
nd <- expand_grid(
  pthn = seq(min(responses_coded$pthn, na.rm = TRUE), max(responses_coded$pthn, na.rm = TRUE), by = 0.1),
  onset = unique(responses_coded$onset),
  lv = c(-1, 1)
)

m <- add_fitted_draws(nd, fit_responses, n = 20, re_formula = NA) %>% 
  mutate_at(vars(lv), function(x) paste0(x, " SD")) %>% 
  mutate(onset = paste0("Onset: ", onset))
...
ggplot(m, aes(pthn, .value, colour = lv, fill = lv)) +
  ...
  labs(x = "Phonological neighbourhood density\n(higher frequency neighbours)",
       y = "P(Correct)", colour = "Phonological similarity",
       fill = "Phonological similarity") +
...
##### By phonological similarity similarity
...
nd <- expand_grid(
  ...
  onset = unique(responses_coded$onset),
  lv = seq(
    min(responses_coded$lv, na.rm = TRUE),
    max(responses_coded$lv, na.rm = TRUE),
    by = 0.1
  )
)

m <- add_fitted_draws(nd, fit_responses, n = 20, re_formula = NA) %>% 
  mutate_at(vars(pthn), function(x) paste0(x, " SD")) %>% 
  mutate(onset = paste0("Onset: ", onset))

ggplot(m, aes(lv, .value, colour = pthn, fill = pthn)) +
  facet_wrap(~onset) +
  # stat_lineribbon(.width = 0.95, alpha = 0.25, size = 0) +
  stat_summary(fun = mean, geom = "line", size = 1) +
  labs(x = "Consonant similarity (SD)",
       y = "P(Correct)", colour = "PTHN",
