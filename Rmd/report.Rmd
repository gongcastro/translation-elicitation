...
m <- add_fitted_draws(nd, fit_responses, n = 50, re_formula = NA) %>% 
...
ggplot(m, aes(lv, .value, colour = pthn, fill = pthn)) +
  stat_lineribbon(.width = 0.95, alpha = 0.25, size = 0) +
  stat_summary(fun = mean, geom = "line", size = 1) +
  labs(x = "Phon. similarity (SD)", y = "P(Correct)", colour = "Phon. neigh. dens.",
       fill = "Phon. neigh. dens.") +
  scale_y_continuous(labels = scales::percent, limits= c(0, 1)) +
  guides(colour = guide_legend(ncol = 2)) +
  theme_custom() +
  theme(
  ...
  ggsave(here("Figures", "responses_lv.png"), height = 4, width = 4.5)
...
summary(fit_jtrace$fit)$fixed %>% 
  ...
  tab_footnote(
    footnote = "Transformed using the divide-by-four- rule to get the maximum change in probability of correct response, associated with a unit increase in this variable.",
    locations = cells_body(columns = "term", rows = term %in% c("pthn", "frequency", "lv", "pthn:lv") | str_detect(term, "activation"))
  ) %>% 
  tab_footnote(
    footnote = "ESS: Effective sample size",
    locations = cells_column_labels(columns = c("Bulk_ESS", "Tail_ESS"))
...
ggplot(d_jtrace, aes(response_reordered, activation, fill = correct)) +
  facet_wrap(input_label~source, scales = "free_y", ncol = 3) +
