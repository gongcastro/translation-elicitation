...
```{r appendix-model-code, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
model_fits$fit_3$model
color_scheme_set(c("#1A85FF", "#9ccaff", "#1A85FF",  "#1A85FF", "#1A85FF", "#1A85FF"))
...
```{r appendix-mcmc-diagnostics, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
posterior_cp <- as.array(model_fits$fit_3)

plot_pairs <- mcmc_pairs(
  posterior_cp, pars = c("b_Intercept", "b_frequency_zipf", "b_pthn", "b_lv", "b_pthn:lv"),
...
```{r appendix-trace-plot}
mcmc_trace(model_fits$fit_3, pars = c("b_Intercept", "b_frequency_zipf", "b_pthn", "b_lv", "b_pthn:lv"), n_warmup = 0) +
  annotate(geom = "rect", xmin = -Inf, xmax = 500, ymin = -Inf, ymax = Inf, alpha = 0.5, fill = "grey")
