...
lengths <- stimuli %>% 
    ...
    mutate_at(vars(ipa_flat_1:word_2), nchar) %>% 
    group_by(group) %>% 
    summarise_at(
        vars(ipa_flat_1, ipa_flat_2, word_1, word_2), 
...
pthn_example <- clearpond_path %>% 
    ...
    bind_rows(.id = "group") %>% 
    clean_names() %>% 
    filter(word %in% c("book", "sock"), group=="cat-ENG") %>% 
    select(word, pthn) 
...
#| fig-width: 7
#| fig-height: 5.5
#| fig-cap: "Distribution and summary statistics of target translations, plotted in blue for English translations of Catalan stimuli (cat-ENG), pink for English translations of Spanish stimuli (spa-ENG), and yellow for Spanish translation of Catalan stimuli (cat-SPA). Columns indicate: A\\) Lexical frequency of target translations, expressed as Zipf scores. B\\) PTHN of target translations, and C\\) Normalised Levenshtein phonological similarity between the presented words and their target translations, expressed in percentages. Lexical frequencies and PTHN scores of the presented words are not used in our analyses, as participants lack any lexical representation of such words."
d <-  mutate(stimuli, group = factor(group, levels = c("cat-ENG", "spa-ENG", "cat-SPA"))) 
...
d %>% 
    ...
    geom_text(
        data = medians, 
        aes(label = "Median ± SD", x = frequency_zipf_median+1.25, y = 13), 
        size = 3
    ) +
    geom_curve(
        data = medians, 
        aes(x = frequency_zipf_median+0.85, xend = frequency_zipf_median+0.1, y = 12, yend = 11),
    ...
    geom_rect(
        data = medians, 
        aes(xmin = 0, xmax = pthn_median+pthn_sd, ymin = -Inf, ymax = Inf, fill = group),
        inherit.aes = FALSE, alpha = 0.15
    ) +   
    geom_col(colour = "white") + 
    geom_vline(
        data = medians, 
        aes(xintercept = pthn_median, group = group), 
        colour = "black", size = 0.5, linetype = "dashed"
    ) + 
    labs(
        x = "PTHN", 
...
tab_loos <- model_loos %>% 
    ...
    gt(rowname_col = "model", id = "model-comparison") %>% 
    tab_header(
        title = md("Table 2"),
