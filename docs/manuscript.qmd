...
All analyses were performed in the R environment [@rcoreteam2013language]. We used the tidyverse family of R packages [@wickham2019welcome] to process data and to generate figures. We used the `brms` R package [@burkner2017brms] using the `cmdstanr` back-end to the Stan probabilistic language [@carpenter2017stan] to estimate and compare the models (see #apx-diagnostics for model diagnostics). Numeric predictors were standardised before entering the model by subtracting the mean and dividing by the standard deviation.
...
## Appendix 1: Model diagnostics {#apx-diagnostics}
...
```{r}
#| label: apxfig-appendix-diagnostics
#| fig-cap: "Traceplots of fixed regression coefficients of models in Experiments 1, 2, and 3."
#| fig-width: 9
#| fig-height: 9
var_labels <- c(
...
## Appendix 2: Pooled analyses of Experiments 1 and 3 {#apx-pooled}
...
```{r}
#| label: apxfig-coefs
#| fig-cap: "Regression coefficients across Experiments 1, 2, and 3."
#| fig-width: 9
#| fig-height: 5
var_labels <- c(
  "b_Intercept" = "Intercept",
