...
stimuli_coded %>% 
  count(group, pthn) %>% 
  ggplot(aes(reorder(as.factor(pthn), -n), n, fill = group)) +
  facet_wrap(~group, ncol = 1) +
  geom_col() +
  labs(x = "number of phonological neighbours with higher lexical frequency\nExtracted from CLEARPOND database", y = "# trials", fill = "Test language") +
  theme(
    legend.position = "top",
    panel.grid.major.y = element_line(colour = "grey", linetype = "dotted")
  )
...
ggplot(post, aes(.value, .variable)) +
  ...
  stat_halfeye(point_colour = "white", interval_colour = "white", slab_fill = "grey80", .width = 0.95) +
