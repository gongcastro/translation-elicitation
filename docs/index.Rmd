...
stimuli_coded <- stimuli %>% 
  mutate(test_language = ifelse(group=="spa-ENG", "Spanish", "Catalan"))
```
...
nd_re_words <- expand.grid(
  translation_id = unique(fit_6$data$translation_id),
  pthn_std = c(-1, 1),
...
post_re <- add_epred_draws(
  ...
  ungroup() %>% 
  select(-group) %>% 
  left_join(select(stimuli, group, translation, translation_id))

post_re %>% 
  ggplot() +
  aes(
    .epred,
    reorder(as.factor(translation), .epred)
  ) +
