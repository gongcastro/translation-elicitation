...
* fit_0: `~ 1 + (1 | participant_id) + (1 | translation_id)`
* fit_1: `~ 1 + frequency_zipf + (1 + frequency_zipf | participant_id) + (1 | translation_id)`
* fit_2: `~ 1 + frequency_zipf + pthn + (1 + frequency_zipf + pthn | participant_id) + (1 | translation_id)`
* fit_3: `~ 1 + frequency_zipf + pthn + lv + (1 + frequency_zipf + pthn + lv | participant_id) + (1 | translation_id)`
* fit_4: `~ 1 + frequency_zipf + pthn + lv + pthn:lv + (1 + frequency_zipf + pthn + pthn:lv + lv | participant_id) + (1 | translation_id)`
...
post_sd_data <- posterior_draws_fixed %>% 
  ...
  mutate(
    .variable_name = str_remove(
      .variable, 
      "sd_participant_id__|sd_translation_id__"
    ) %>% 
      str_replace_all(str_repl) %>% 
      factor(
        levels = c(
    ...
    group = case_when(
      ...
      str_detect(.variable, "translation") ~ "Words"
    ),
    .value = ifelse(
      str_detect(.variable, "Intercept"),
      inv_logit_scaled(.value),
...
corr_mat <- VarCorr(fit_4, summary = TRUE)$participant_id$cor 
corr_names <- rownames(VarCorr(fit_4, summary = TRUE)$participant_id$cor)
...
post_cors <- corr_data %>% 
  ...
  theme(
    ...
    axis.text.x = element_text(size = 8, angle = 90, hjust = 0, vjust = 1),
...
fixed_intercept <- fixef(fit_4)[1,c(1, 3, 4)] %>% 
  inv_logit_scaled()

empirical_accuracy <- responses %>% 
  group_by(group, translation_id, lv) %>% 
  summarise(
...
post_re %>% 
  left_join(
    select(
      ...
      mutate(translation = paste0(translation, " (", percent(lv), ")"))
  ) %>% 
  ggplot(
    aes(
      x = .value,
      y = reorder(
        as.factor(translation), 
        .value
      )
    )
  ) +
  ...
  labs(
    ...
    y = "Translation",
    colour = "Model prediction (credible interval)",
    caption = "Credible intervals cover the range of values of the sampling space that contain the true value with a given certainty."
  ) +
  scale_color_manual(
...
```{r post_re_word_lv, echo=FALSE, fig.height=9, fig.width=7, message=FALSE, warning=FALSE, paged.print=FALSE}
...
post_re %>% 
  ...
  labs(
    ...
    y = "Translation (Levenshtein similarity)",
    colour = "Model prediction (credible interval)",
    caption = "Credible intervals cover the range of values of the sampling space that contain the true value with a given certainty."
  ) +
  scale_color_manual(
...
nd <- expand.grid(
  lv_std = c(
    min(fit_4$data$lv_std),
    0,
    max(fit_4$data$lv_std)
  ),
  frequency_zipf_std = 0,
  pthn_std = seq(
    min(fit_4$data$pthn_std, na.rm = TRUE),
    max(fit_4$data$pthn_std, na.rm = TRUE),
    by = 0.1
  )
)
...
stimuli %>% 
    ...
    fmt_number(frequency_median:pthn_sd) %>%
    fmt_percent(matches("lv")) %>% 
    tab_spanner(md("**Freq. (million)**"), c("frequency_median", "frequency_sd")) %>% 
...
ggsave(here("img", "accuracy_empirical.png"), height = 9, width = 15, dpi = 500)
```
...
familiarity <- stimuli %>% 
    ...
    mutate(
        ...
        test_language = case_when(
            group=="ENG-CAT" ~ "Catalan",
            group=="ENG-SPA" ~ "Spanish",
            group=="SPA-CAT" ~ "Catalan"
        ),
    ...
    group_by(group) %>% 
    mutate(
        word_reorder = reorder_within(word, frequency_zipf, group)
    ) %>% 
    ungroup()
